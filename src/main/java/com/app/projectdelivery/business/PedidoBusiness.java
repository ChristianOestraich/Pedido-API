...
public class PedidoBusiness {
    ...
    public static ResponseEntity<String> processarPedidos(List<PedidoModel> pedidos) {
        ...
        try {
            pedidos.stream()
                    .peek(pedido -> {
                        ...
                        if (numerosControleSet.size() >= 10) {
                            throw new IllegalStateException("Limite de 10 pedidos excedido.");
                        }

                        // Regra 2: Não aceitar número de controle já cadastrado
                        if (!numerosControleSet.add(pedido.getNumero())) {
                            throw new IllegalStateException("Número de controle duplicado: " + pedido.getNumero());
                        }
                    })
                    .forEach(pedido -> {
            ...
            // Captura exceções geradas pelas regras e retorna uma resposta de erro
